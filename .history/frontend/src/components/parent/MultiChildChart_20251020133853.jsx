...
const MultiChildChart = ({ 
  ...
  // Move all hooks to the top - no conditional hooks
  ...
  const childKeys = useMemo(() => {
    ...
    // Get all unique keys that represent children (excluding date)
  ...
  // Debug logging
  console.log("MultiChildChart - Real API data:", chartData);
  ...
  if (!chartData.length) {
    return (
      <Card className="pd-card h-100">
        <Card.Body>
          <div className="d-flex justify-content-center align-items-center flex-column" style={{ height: `${height}px` }}>
            <div className="text-muted mb-2">No activity data available</div>
            <small className="text-muted">Add children or wait for activity data to appear</small>
          </div>
        </Card.Body>
      </Card>
    );
