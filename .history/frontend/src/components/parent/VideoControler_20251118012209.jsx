...
const VideoController = () => {
  ...
  const confirmSave = async () => {
    ...
    try {
      ...
      const assignmentPromises = selectedItems.map(async (item) => {
        const assignData = {
          child_id: parseInt(selectedChild.id), // Convert to number
      ...
      const errorMessage = err.message || "Failed to assign content. Please check if the backend server is running.";
      setError(errorMessage);
      console.error("Error assigning content:", err);
      
      // Check if server went offline during the request
  ...
  const retryConnection = async () => {
    ...
    // Refetch data if server is back online
    if (serverOnline) {
      const userData = JSON.parse(localStorage.getItem("user") || "{}");
      const parentId = userData.id || userData.parentId || "";
      if (parentId) {
    ...
    <div className="container mt-4">
      ...
      <div className="d-flex justify-content-between align-items-center mb-3">
        ...
        <div className="d-flex align-items-center">
          <div className={`badge ${serverOnline ? 'bg-success' : 'bg-danger'} me-2`}>
              ...
              {selectedChild && (
                <div className="bg-light p-3 rounded">
                  ...
                  <p className="mb-0 fw-bold text-primary">
                    <i className="bi bi-person-check-fill"></i> {selectedChild.name} (ID: {selectedChild.id})
                  </p>
                </div>
              )}
            </div>
