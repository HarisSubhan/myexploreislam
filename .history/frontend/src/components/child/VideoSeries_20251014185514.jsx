...
import { getSeriesApi } from "../services/config"; // Adjust import path as needed
...
const VideoSeries = () => {
  ...
  useEffect(() => {
    const fetchSeriesData = async () => {
      try {
        setLoading(true);
        const seriesData = await getSeriesApi();

        // Transform API data to match your seasons format
        const transformedSeasons = seriesData.map((series, index) => ({
          id: series.id,
          title: series.series_title || `Season ${index + 1}`,
          description: series.series_description,
          color: getSeasonColor(index), // Helper function for colors
