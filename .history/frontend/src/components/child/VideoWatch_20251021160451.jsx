...
const VideoWatch = () => {
  ...
  // Build correct video URL - FIXED for your API response
  const buildVideoUrl = (videoUrl) => {
    if (!videoUrl) return null;

    // If URL is already absolute (includes baseUrl from API response)
  ...
  // Build thumbnail URL - FIXED for your API response
    ...
    const fetchVideoData = async () => {
      try {
        ...
        if (videoData) {
          ...
          // Get recommended videos only for single videos
          if (!seriesSlug && !videoData.series_id) {
  ...
  const handleBackToSeries = () => {
    if (seriesSlug) {
      ...
      // Extract series slug from current video
      const seriesSlugFromVideo =
        createSlug(currentVideo.series_name) ||
        `series-${currentVideo.series_id}`;
  ...
  return (
    <Container fluid className="py-4" style={{ maxWidth: "1400px" }}>
      ...
      <Row>
        ...
        {videoType === "single" && recommendedVideos.length > 0 && (
          <Col lg={4}>
