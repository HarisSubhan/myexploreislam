...
import { dashboardApi } from "../../services/dashboardApi"; // Import the dashboardApi
...
const VideoWatch = () => {
  ...
  // Get child_id from localStorage or wherever you store child info
  const getChildId = () => {
    // You might need to adjust this based on how you store child data
  ...
  const logVideoWatch = async (video) => {
    try {
      ...
      // Don't show error to user as this shouldn't interrupt video playback
    }
  };
    ...
    const fetchVideoData = async () => {
      try {
        ...
        if (videoId) {
          ...
          // Log video watch when video data is fetched
          if (videoData) {
            logVideoWatch(videoData);
          }
        }
  ...
  const handleRecommendedClick = async (recommendedVideoId) => {
    try {
      // Find the recommended video data to log before navigation
      const recommendedVideo = recommendedVideos.find(
        (video) => video.id.toString() === recommendedVideoId.toString()
      );
