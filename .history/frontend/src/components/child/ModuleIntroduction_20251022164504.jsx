...
import { getAllVideosApi, getVideoByIdApi } from "../../services/videoApi"; // ✅ Add single video API
import { createSlug } from "../../utils/slugify";

const ModuleIntroduction = () => {
  const navigate = useNavigate();
  const { seriesSlug, videoId } = useParams(); // ✅ Get videoId for single videos
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // ✅ Check if this is a single video flow
        const isSingleVideoFlow = location.pathname.includes("/module/single/");
        setIsSingleVideo(isSingleVideoFlow);

        console.log(
          "🔄 Flow type:",
          isSingleVideoFlow ? "Single Video" : "Series"
        );
        console.log(
          "🔄 Fetching data for:",
          isSingleVideoFlow
        ...
        if (isSingleVideoFlow) {
          // ✅ SINGLE VIDEO FLOW
          await fetchSingleVideoData();
        } else {
          // ✅ SERIES FLOW (your existing code)
    ...
    const fetchSingleVideoData = async () => {
      // For single videos, we only need the video data
      let videoToUse = null;

      // Priority 1: Use video from location state
      if (videoDataFromState) {
        videoToUse = videoDataFromState;
        console.log("🎯 Using video from location state:", videoToUse.title);
      }
      // Priority 2: Fetch video by ID from URL
      else if (videoId) {
        console.log("🔄 Fetching single video by ID:", videoId);
        videoToUse = await getVideoByIdApi(videoId);
        console.log("✅ Found single video:", videoToUse?.title);
      }

      if (videoToUse) {
        setSelectedVideo(videoToUse);
        // For single videos, we can create a mock series object
        setSeries({
          name: videoToUse.title,
          description: videoToUse.description || "Single video module",
          id: videoToUse.id,
        });
        setVideos([videoToUse]); // Single video as the only one in the list
      } else {
    ...
    const fetchSeriesData = async () => {
      // ✅ SERIES FLOW - Your existing code (unchanged)
      if (!seriesSlug) {
      ...
      if (videoDataFromState) {
        ...
        console.log("🎯 Using video from location state:", videoToSelect.title);
      }
    ...
    fetchData();
  }, [seriesSlug, videoId, location, videoDataFromState, videoIdFromState]);

  // ✅ UPDATED: Working navigation function for both flows
  const handleContinue = () => {
    console.log("🎬 Continue button clicked");
    console.log("📍 Selected video:", selectedVideo);
    console.log("📍 Is single video:", isSingleVideo);
    ...
    if (isSingleVideo) {
      // ✅ SINGLE VIDEO FLOW NAVIGATION
      console.log(
        "➡️ Navigating to single video page1:",
        `/child/module/single/${selectedVideo.id}/page1`
      );
      ...
      // ✅ SERIES FLOW NAVIGATION (your existing code)
      if (!seriesSlug) {
  ...
  // ✅ Handle video selection (for series flow with multiple videos)
  ...
  const handlePlayAudio = () => {
    const title = isSingleVideo ? selectedVideo?.title : series?.name;
    ...
    >
      ...
      <div className="text-center mb-5">
        ...
        <h1 className="fw-bold mb-3" style={{ color: "#0d6efd" }}>
          Welcome to the {isSingleVideo ? "Video" : "Series"}!
        </h1>
        ...
        <Col md={10} lg={8}>
          ...
          >
            <Card.Body className="p-5">
              {/* Video Selection Section - Only show for series with multiple videos */}
              ...
              >
                {isSingleVideo
                  ? selectedVideo?.title?.toUpperCase() || "VIDEO INTRODUCTION"
                  : series?.name?.toUpperCase() || "SERIES INTRODUCTION"}
              </h3>
                ...
                {isSingleVideo ? (
                  ...
                  >
                    {selectedVideo?.description ||
                      "Welcome to this single video module! In this video, you'll learn important concepts through engaging content. Get ready to enhance your knowledge!"}
                  </p>
                ) : series?.description ? (
                  <div
                    style={{
                  ...
                  <div className="d-flex align-items-center">
                    ...
                    <div>
                      <h6 className="fw-bold mb-1">
                        Engaging {isSingleVideo ? "Video" : "Videos"}
                      </h6>
                      <small className="text-muted">
              ...
              <div className="d-flex justify-content-between align-items-center mt-4 pt-4 border-top">
                ...
                {/* ✅ UPDATED: Working Continue Button */}
                <Button
          ...
          <Card className="mt-4 border-warning">
            <Card.Body>
              <small className="text-muted">
                ...
                <br />
                Flow: {isSingleVideo ? "Single Video" : "Series"} | <br />
