...
const VideoWatch = () => {
  const { moduleSlug } = useParams(); // Now using slug instead of videoId
  const { moduleSlug, videoSlug } = useParams(); // Now using slugs
  ...
  useEffect(() => {
    const fetchVideoData = async () => {
      try {
        ...
        // Extract video ID from slug or use alternative method
        // You might need to modify your API to support slug-based fetching
        let videoData;

        if (videoSlug) {
          // Try to extract ID from slug (if using ID-Slug combination)
          const videoIdMatch = videoSlug.match(/-(\d+)$/);
          if (videoIdMatch) {
            const videoId = videoIdMatch[1];
            videoData = await getVideoByIdApi(videoId);
          } else {
            // Fallback: Get all videos and find by slug
            const allVideos = await getAllVideosApi();
            videoData = allVideos.find(
              (video) => createSlug(video.title) === videoSlug
            );
          }
        }
  ...
  const handleRecommendedClick = async (recommendedVideo) => {
    try {
      await logVideoWatch(recommendedVideo);

      // Navigate with slugs
      const moduleSlug = createSlug(
        recommendedVideo.module_name || `module-${recommendedVideo.module_id}`
      );
      const videoSlug = createSlug(
        recommendedVideo.title || `video-${recommendedVideo.id}`
      );

      navigate(`/child/video/${moduleSlug}/${videoSlug}`);
    } catch (error) {
  ...
  // Rest of the VideoWatch component remains similar...
  // Just update the navigation and URL handling
