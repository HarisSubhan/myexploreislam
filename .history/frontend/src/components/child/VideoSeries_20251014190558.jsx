...
import { baseUrl } from "../../services/config"; // Import baseUrl
  ...
  useEffect(() => {
    const fetchSeriesData = async () => {
      try {
        ...
        console.log("API Response:", videosData); // Debug log
        ...
        const transformedSeasons = videosData.map((series, index) => ({
          ...
          // Fix the thumbnail URL by prepending baseUrl if it's a relative path
          img: series.thumbnail_url
            ? series.thumbnail_url.startsWith("http")
              ? series.thumbnail_url
              : `${baseUrl}${series.thumbnail_url}`
            : "/assets/images/book.png",
        }));

        console.log("Transformed Seasons:", transformedSeasons); // Debug log
          ...
          {seasons.map((season, index) => (
            <Col xs={12} sm={6} md={4} key={season.id}>
              ...
              >
                ...
                >
                  {/* Show title only if no image or image fails to load */}
