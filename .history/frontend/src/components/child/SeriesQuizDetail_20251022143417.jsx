...
const SeriesQuizDetail = () => {
  ...
  // In SeriesQuizDetail component - handleVideoClick
  const handleVideoClick = (video) => {
    console.log("🎬 Starting module for video:", video.title);

    // ✅ Navigate with videoId in URL parameter
  ...
  // Also update handleStartSeries
  const handleStartSeries = () => {
    console.log("🎬 Starting series:", series.name);

    if (videos.length > 0) {
      const firstVideo = videos[0];
      navigate(`/child/module/${seriesSlug}/page1/${firstVideo.id}`, {
        state: {
  ...
  // Remove or modify handleStartSeries to also go directly to page1
  const handleStartSeries = () => {
    console.log("🎬 Starting series:", series.name);
