...
const ModuleIntroduction = () => {
  ...
  console.log("📍 Is Single Video from State:", isSingleVideoFromState);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Check both location state AND URL pattern
        const isSingleVideoFlow =
          isSingleVideoFromState ||
          (location.pathname.includes("/module/single/") && videoId);

        console.log("🔍 Single Video Flow Detection:", {
          fromState: isSingleVideoFromState,
          fromURL: location.pathname.includes("/module/single/"),
          hasVideoId: !!videoId,
          finalResult: isSingleVideoFlow,
        });
        ...
        if (isSingleVideoFlow) {
          ...
          if (videoData) {
            ...
            console.error("❌ Could not load single video data");
          }
          ...
          <Card className="mt-4 border-warning">
            <Card.Body>
              <small className="text-muted">
                ...
                Source: {videoDataFromState ? "State" : "Direct URL"} | <br />
