  ...
  const fetchTickets = async () => {
    try {
      ...
      const transformedTickets = response.data?.map(ticket => ({
        ...
        user: {
          name: ticket.parent_id?.name || ticket.parent_id?.username || "Unknown User",
          email: ticket.parent_id?.email || "No email",
          ...
          <Card.Body className="p-0">
            {filteredTickets.length > 0 ? (
              ...
              <div className="text-center py-5">
                ...
                <p className="text-muted">
                  {tickets.length === 0 ? "No tickets found" : "No tickets found matching your filters"}
                </p>
                {tickets.length === 0 ? (
                  <Button variant="primary" onClick={fetchTickets}>
                    Try Again
