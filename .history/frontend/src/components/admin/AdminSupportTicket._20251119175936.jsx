...
const AdminSupportTicket = () => {
    ...
    try {
      ...
      const transformedTickets =
        response.tickets?.map((ticket) => {
          // Get user name from available fields
          const getUserName = () => {
          ...
          return {
            ...
            user: {
              name: userName,
              // Remove email field since it's not in API response
              type: "parent",
            },
            ...
            updatedAt: ticket.updated_at
              ...
              : new Date().toLocaleString(),
            lastReply: calculateLastReply(ticket.updated_at || ticket.updatedAt || ticket.created_at || ticket.createdAt),
    ...
    if (searchTerm) {
      result = result.filter(
        (ticket) =>
          ...
          // Remove email search since it's not in API
      );
    }
  ...
  // Export CSV function - updated to remove email
  const exportToCSV = () => {
