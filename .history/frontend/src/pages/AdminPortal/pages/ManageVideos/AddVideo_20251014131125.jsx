...
const AddVideo = () => {
  ...
  const [seriesList, setSeriesList] = useState([]); // Initialize as empty array
  ...
  useEffect(() => {
    const fetchSeries = async () => {
      try {
        ...
        // Fix: Check the actual structure of your response
        // If response is already the array, use response directly
        // If it's response.data, use that
        const seriesData = Array.isArray(response) ? response : response.data;
        setSeriesList(seriesData || []); // Ensure it's always an array
      } catch (error) {
        console.error("Error fetching series:", error);
        setSeriesList([]); // Set empty array on error
      }
    };
    fetchSeries();
  }, []);
              ...
              {videoType === "single" ? (
                ...
                <Form.Group className="mb-3">
                  ...
                  <Form.Control
                    ...
                    accept="video/*"
                    multiple
                    onChange={(e) => setSeriesVideos(Array.from(e.target.files))}
                    required
                  />
                </Form.Group>
              )}
