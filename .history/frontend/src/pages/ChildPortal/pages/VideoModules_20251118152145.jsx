  ...
  useEffect(() => {
    const fetchAssignedContent = async () => {
      try {
        ...
        // Handle the actual API response format based on your API structure
        let assignedContent = [];
        
        if (response && response.data && Array.isArray(response.data)) {
          // Use the data array from the response (your actual API structure)
        ...
        assignedContent.forEach((item) => {
          ...
          if (item.video_id && item.video_title) {
            formattedSingleVideos.push({
              ...
              progress: 0, // You might need to fetch this separately
              video_url: "", // You might need to fetch this separately
            });
          }
