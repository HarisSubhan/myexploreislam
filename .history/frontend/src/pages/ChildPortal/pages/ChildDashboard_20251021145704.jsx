...
const ChildDashboard = () => {
      ...
      try {
        ...
        const formattedModules = data.map((module) => {
          ...
          return {
            ...
            // Add videos array if available from API
  ...
  // DIRECT VIDEO PLAY FUNCTION
  const handleCardClick = async (module) => {
    const moduleSlug = module.slug;

    if (module.type === "series") {
      // Series ke liye series page par navigate karenge
      navigate(`/child/series/${moduleSlug}`);
    } else {
      // Single video ke liye directly video play karenge
      try {
        // Agar module ke saath videos array available hai to first video play karenge
        if (module.videos && module.videos.length > 0) {
          ...
          // Fallback: Agar videos array nahi hai to module ID use karenge
          navigate(`/child/singles/${module.id}`);
        }
      } catch (error) {
  ...
  // Unauthorized State - same as before
  if (unauthorized) {
    return (
      <Container
        fluid
