...
const ChildDashboard = () => {
  ...
  // FIXED: Handle card click properly
  const handleCardClick = (module) => {
    const moduleSlug = module.slug || createSlug(module.title);

    if (module.type === "series") {
      // For series: navigate to series detail page
      navigate(`/child/series/${moduleSlug}`);
    } else {
      // For single videos: navigate directly to video player
      navigate(`/child/browse/singles/${moduleSlug}`);
    }
  };

  // Alternative: If you want to use module routes for series
  const handleSeriesModuleClick = (module) => {
    const moduleSlug = module.slug || createSlug(module.title);
    
    if (module.type === "series") {
      // Navigate to module introduction for structured learning
      navigate(`/child/module/series/${module.id}/introduction`);
    } else {
      // For single videos: navigate directly to video player
      navigate(`/child/browse/singles/${moduleSlug}`);
    }
