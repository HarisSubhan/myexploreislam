...
const childApi = {
  ...
  getChildren: async (parentId) => {
    ...
    const response = await fetch(`http://localhost:5000/api/parent/children/${parentId}`, {
      method: "GET",
      headers: {
  ...
  updateChild: async (childId, childData) => {
    const token = localStorage.getItem("token");
    const response = await fetch(`http://localhost:5000/api/parent/child/${childId}`, {
      method: "PUT",
      headers: {
        "Authorization": `Bearer ${token}`,
        "Content-Type": "application/json",
      ...
      <div style={{ position: "fixed", right: 20, bottom: 20, zIndex: 1100 }}>
        <Toast show={showUndoToast} onClose={() => setShowUndoToast(false)}>
          ...
          <Toast.Body className="d-flex align-items-center gap-2">
            ...
            <Button size="sm" variant="outline-primary" onClick={undoDelete} disabled={apiLoading}>
...
const ChildFormModal = ({ show, onHide, onSave, editing, loading }) => {
  ...
  const validate = () => {
    ...
    if (!editing?.id) {
      ...
      if (form.password !== confirmPassword) err.confirmPassword = "Passwords do not match";
    }
            ...
            <Col md={6}>
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  ...
                  placeholder={editing?.id ? "Enter new password" : "Enter password"}
                />
                <Form.Control.Feedback type="invalid">
                  {errors.password}
                </Form.Control.Feedback>
            ...
            {child.subjects?.map((subject) => (
              <div key={subject.name} className="mb-2">
                ...
                <ProgressBar 
                  now={subject.score} 
                  variant={
                    subject.score > 85 ? "success" :
                    subject.score > 70 ? "warning" : "danger"
                  }
                />
              </div>
            ))}
