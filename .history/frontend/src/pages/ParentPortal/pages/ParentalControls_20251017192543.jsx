...
import { useUser } from "../context/UserContext"; // Adjust import path to your UserContext
...
const ParentalControls = () => {
  ...
  const fetchChildren = async () => {
    ...
    // Get parent ID from user object - try different possible properties
    const parentId = user.parent_id || user.parentId || user.id || user.user_id;

    if (!parentId) {
      setError("Parent ID not found in user data. Please contact support.");
      setLoading(false);
      return;
    }
      ...
      setChildren(
        Array.isArray(childrenData)
          ? childrenData.map((child) => ({
              ...
              last_used:
                child.last_used || child.updated_at || child.created_at,
            }))
          : []
      );
    } catch (err) {
  ...
  const confirmToggleLock = async () => {
    ...
    try {
      ...
      setError(
        err.message ||
          `Failed to ${selectedChild.is_active ? "lock" : "unlock"} account`
      );
    } finally {
      setUpdating(null);
  ...
  const handleScreenTimeChange = async (childId, newScreenTime) => {
    try {
      ...
      setChildren((prevChildren) =>
        prevChildren.map((child) =>
          child.id === childId ? { ...child, screenTime: newScreenTime } : child
        )
      );
            ...
            <Card.Body>
              {children.length === 0 ? (
                <div className="text-center text-muted py-4">
                  ...
                  <small>
                    Add children to enable account locking features.
                  </small>
                </div>
              ) : (
