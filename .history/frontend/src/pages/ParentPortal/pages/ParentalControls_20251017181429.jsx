...
const ParentalControls = () => {
  ...
  const fetchChildren = async () => {
    try {
      setLoading(true);
      // Replace 'parentId' with actual parent ID from your auth context or props
      const parentId = 1; // You should get this from your authentication context
      ...
      const childrenData = response.data.map((child) => ({
        id: child.id,
        name: child.name,
        screenTime: child.screen_time || 2, // Default to 2 hours if not provided
        locked: !child.is_active, // Assuming is_active 0 means locked, 1 means unlocked
