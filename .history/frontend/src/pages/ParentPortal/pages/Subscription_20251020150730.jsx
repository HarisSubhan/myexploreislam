...
import { getsubscriptionsAllActiveApi, getsubscriptionsParentByidApi } from "../../../services/subscribeApi";


export default function SubscriptionWidget({ loading: externalLoading }) {
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const { parentId } = useUser(); // Get parentId from context
  ...
  useEffect(() => {
    const fetchSubscriptionData = async () => {
      try {
        ...
        if (parentId) {
          // Fetch specific parent's subscription using the dynamic parentId from context
