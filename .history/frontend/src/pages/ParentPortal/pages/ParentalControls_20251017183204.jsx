...
import { FaLock, FaUnlock, FaClock, FaExclamationTriangle, FaSync } from "react-icons/fa";
import { getChildrenByParentIdApi, updateChildStatusApi } from "../services/api";
...
const ParentalControls = () => {
    ...
    try {
      ...
      const transformedChildren = childrenData.map((child, index) => {
        ...
        const childData = {
          ...
          name: child.name || child.username || child.full_name || `Child ${index + 1}`,
          screenTime: child.screen_time || child.screenTime || child.daily_limit || 2,
          // Assume active if not specified, locked = !is_active
          is_active: child.is_active !== undefined ? child.is_active : 
                    child.active !== undefined ? child.active : 
                    child.status === 'active' ? true : 
                    child.locked !== undefined ? !child.locked : true,
        };
  ...
  const toggleLock = async (id) => {
    try {
      ...
      console.log(`Updating child ${id} (${child.name}) to locked: ${newLockStatus}, is_active: ${isActive}`);
  ...
  return (
    <Container fluid className="py-4">
      ...
      {error && (
        <Alert variant={children.length === 0 ? "warning" : "danger"} className="mb-4">
          <FaExclamationTriangle className="me-2" />
          {error}
          {children.length === 0 && (
            <div className="mt-2">
              <Button variant="primary" size="sm" onClick={fetchChildren} className="me-2">
                Try Again
              </Button>
              <small className="text-muted">
                If the issue persists, check if you have added any children to your account.
              </small>
            </div>
          )}
        </Alert>
            ...
            <Card.Body>
              ...
              {children.length > 0 ? (
                children.map((child) => (
                  <div key={child.id} className="mb-3">
                    ...
                    <ProgressBar
                      ...
                      label={child.locked ? "0h / 4h" : `${child.screenTime}h / 4h`}
                      className="mt-1"
                ...
                <div className="text-center py-4">
                  <p className="text-muted mb-3">No children accounts available</p>
          ...
          <Card className="shadow-sm border-0 rounded-3">
            <Card.Body>
              <div className="d-flex justify-content-between align-items-center mb-3">
                ...
                <span className="badge bg-primary">{children.length} children</span>
              </div>
              ...
              {children.length > 0 ? (
                ...
                <div className="text-center py-5">
                  ...
                  <div>
                    <Button variant="primary" onClick={fetchChildren} className="me-2">
                      Refresh
                    </Button>
