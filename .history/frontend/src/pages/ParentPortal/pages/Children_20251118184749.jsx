...
import ProgressModal from "../../../components/parent/ProgressModal";
...
const Children = () => {
  ...
  // Add new child via API - FIXED VERSION
  const addChild = async (childData) => {
    try {
      setSubmitting(true);
      
      // Call API - don't use response since it only has message
      await addChildApi(childData);
      
      // Close form immediately
      setShowForm(false);
      
      // Reload children to get updated list with proper IDs
      await loadChildren();
  ...
  // Update existing child via API - FIXED VERSION
  const updateChild = async (childData) => {
    try {
      setSubmitting(true);
      
      // Prepare data for API (remove id from the data sent to API)
      const { id, ...updateData } = childData;
      
      // Call edit API - URL: PUT /api/children/{childId}/edit
  ...
  // Combined upsert function - FIXED VERSION
