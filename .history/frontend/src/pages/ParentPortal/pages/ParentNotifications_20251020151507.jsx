...
import { dashboardApi } from "../../../services/childActivity"; // Adjust import path as needed
...
const ParentNotifications = () => {
    ...
    const fetchNotifications = async () => {
      try {
        ...
        if (activityResponse && activityResponse.data) {
          ...
          // If data has a property that contains the array
          ...
          // If data is an object with results property
            ...
            console.warn("Unexpected API response structure:", activityResponse.data);
        ...
        console.log("Extracted activities:", activities); // Debug log
  ...
  const getFilteredNotifications = () => {
    ...
    return filtered.sort((a, b) => {
      const configA = a.source === "system" ? systemNotificationTypes[a.type] : activityToNotificationMap[a.action];
      const configB = b.source === "system" ? systemNotificationTypes[b.type] : activityToNotificationMap[b.action];
  ...
  const getNotificationConfig = (notification) => {
    if (notification.source === "system") {
      return systemNotificationTypes[notification.type] || systemNotificationTypes.reminder;
    }
  ...
  return (
    <Container fluid className="py-4">
      ...
      <div className="mb-3">
        ...
        {["all", "unread", "alerts", "achievements", "activities"].map((filterType) => (
          <Button
            key={filterType}
                    ...
                    <div className="d-flex justify-content-between align-items-start">
                      <div className="d-flex align-items-start flex-grow-1">
                        ...
                        <div className="flex-grow-1">
                          ...
                          {notification.source === "activity" && notification.metadata && (
                            <div className="mt-1">
                              {parseMetadata(notification.metadata).score && (
                                <Badge bg="success" className="me-2 small">
                                  Score: {parseMetadata(notification.metadata).score}%
                                </Badge>
                              )}
                              {parseMetadata(notification.metadata).duration && (
                                <Badge bg="info" className="me-2 small">
                                  {parseMetadata(notification.metadata).duration}m
                                </Badge>
                              )}
                            </div>
                          )}
