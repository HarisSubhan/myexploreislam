    ...
    <tr>
      ...
      <td>
        <div>
          ...
          {ticket.description && (
            <small className="text-muted d-block text-truncate" style={{ maxWidth: 'min(300px, 30vw)' }}>
              {ticket.description}
            </small>
          )}
        </div>
...
const TicketsTable = React.memo(({ tickets, loading, currentUser, onOpenTicket }) => {
  if (loading) {
    return (
...
const ParentSupports = () => {
  ...
  // Improved auth check function with better user identifier handling
  const checkAuth = useCallback(() => {
    try {
      ...
      if (userData && token) {
        ...
        // Enhanced user identifier check - check multiple possible ID fields
        ...
        if (userId) {
          ...
          console.error("No user identifier found in:", user);
  ...
  const headerContent = useMemo(() => (
    <div className="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3 mb-4">
      ...
      <div className="d-flex align-items-center gap-2 flex-wrap">
        ...
        <Button variant="outline-primary" onClick={refreshAll} disabled={loading} size="sm">
            ...
            <Card className="shadow-sm border-0 rounded-3 h-100">
              <Card.Body>
                ...
                <Accordion flush className="mb-3">
                  {[
                    { key: "0", header: "🔑 How do I reset my password?", body: "Go to Account Settings → Reset Password and follow the steps." },
                    { key: "1", header: "💳 How can I manage my subscription?", body: "Visit Subscriptions & Billing to change your plan or payment method." },
                    { key: "2", header: "🕒 How do I set screen-time limits?", body: "Navigate to Parental Controls and configure screen-time settings for each child." }
                  ].map(({ key, header, body }) => (
        ...
        <Modal.Body className="pt-0">
          {currentUser && (
            <div className="mb-3 p-2 bg-light rounded">
              <small>
                <strong>Creating ticket as:</strong> {currentUser.name} (ID: {currentUser.id})
              </small>
            </div>
          )}
