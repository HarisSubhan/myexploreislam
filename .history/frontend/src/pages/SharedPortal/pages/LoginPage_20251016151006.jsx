...
function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // ENHANCED: Ensure user object has proper ID field for ParentSupports
      const userWithId = {
        ...user,
        // CRITICAL FIX: Ensure we have an 'id' field that ParentSupports can find
        id: user.id || user.userId || user._id || user.user_id,
        name: user.name || user.username || user.email,
        email: user.email || "",
        role: user.role || "user",
      };

      console.log("Storing user data:", userWithId);

      // Store all data in localStorage FIRST
      localStorage.setItem("token", token);
      localStorage.setItem("userRole", user.role);
      localStorage.setItem("user", JSON.stringify(userWithId));

      // CRITICAL FIX: Force synchronous storage
      await new Promise((resolve) => {
        ...
        const checkStorage = () => {
          ...
          if (storedUser && storedToken) {
            console.log("✅ Storage confirmed:", { storedUser, storedToken });
            resolve();
          } else {
      ...
      // Navigate immediately after confirming storage
