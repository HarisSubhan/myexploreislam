...
const RegisterPage = () => {
  ...
  try {
    ...
    // TEMPORARY FIX: Since backend is not returning user_id
    // We'll proceed with checkout using email and hope backend can handle it
    if (registerResponse.message === "Parent registered successfully") {
      console.log("Registration successful, proceeding to checkout...");
      
      // Step 2: Create Stripe Checkout Session with minimal data
      // Backend should be able to find the user by email or handle this differently
      const checkoutData = {
        user_email: form.email, // Backend can use this to find the user
