...
export const submitQuizApi = async (submissionData) => {
  ...
  const response = await axios.post(
    `${baseUrl}/api/quiz-submissions/submit`,
    submissionData,
    {
      headers: {
        Authorization: `Bearer ${token}`,
...
// ✅ FIXED: Get quiz by video ID only (single parameter)
export const getQuizBySeriesVideoApi = async (videoId) => {
  const token = getToken();

  const response = await axios.get(
    `${baseUrl}/api/quizzes/video/${videoId}`, // ✅ Changed URL to accept only videoId
    {
      headers: {
        Authorization: `Bearer ${token}`,
      },
...
// ✅ ALTERNATIVE: Agar backend ko seriesId bhi chahiye, to yeh function use karein
export const getQuizBySeriesAndVideoApi = async (seriesId, videoId) => {
  const token = getToken();

  const response = await axios.get(
    `${baseUrl}/api/quizzes/series/${seriesId}/video/${videoId}`,
    {
      headers: {
        Authorization: `Bearer ${token}`,
      },
