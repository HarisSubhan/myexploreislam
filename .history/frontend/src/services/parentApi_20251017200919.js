...
// Get specific child by parent ID and child ID
export const getChildByParentAndChildIdApi = async (parentId, childId) => {
  const token = getToken();

  if (!token) throw new Error("You are not logged in. Please log in first.");

  try {
    const response = await axios.get(`${baseUrl}/api/parent/child/${childId}`, {
      params: { parent_id: parentId },
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
    console.log("Get Child Response:", response.data);
    return response.data;
  } catch (error) {
