// utils/activityDataTransformer.js
...
export const transformChildrenActivityToChartData = (childrenData = []) => {
  ...
  const chartData = sortedDates.map((date) => {
    ...
    childrenData.forEach((child) => {
      // Create a safe key name from child's name or username
      const childKey = `child_${child.child_id}`;
      const childName = child.name || child.username || childKey;
...
/**
 * Alternative: Group by child names for better readability
 */
export const transformChildrenActivityByChild = (childrenData = []) => {
  if (!childrenData.length) return [];
  ...
  childrenData.forEach((child) => {
    const childName = child.name || child.username || `Child ${child.child_id}`;
    ...
    child.daily_activity?.forEach((activity) => {
      ...
      dayData[childName] = activity.minutes || 0;
    });
  });
