...
// If you need to include the new fields in requests
const createRequest = (
  parentId,
  requestedChildren,
  age,
  ...
  const sql = `INSERT INTO child_requests (parent_id, requested_children, age, profile_image, color) VALUES (?, ?, ?, ?, ?)`;
  db.query(
    sql,
    [parentId, requestedChildren, age, profile_image, color],
    callback
  );
};
