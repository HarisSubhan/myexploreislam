...
const login = (req, res) => {
  ...
  findUserByEmail(email, (err, users) => {
    ...
    if (users.length > 0) {
      ...
      // Step 2: Try to find user in `children` table
      db.query("SELECT * FROM children WHERE email = ?", [email], (err, childResults) => {
        if (err) return res.status(500).json({ error: 'DB error' });
        if (childResults.length === 0) return res.status(401).json({ error: 'Invalid email or password' });

        const child = childResults[0];
        bcrypt.compare(password, child.password, (err, match) => {
          if (err || !match) return res.status(401).json({ error: 'Invalid credentials' });

          const token = jwt.sign({ id: child.id, role: 'child' }, process.env.JWT_SECRET, { expiresIn: '1d' });
