...
const addChild = (req, res) => {
  ...
  if (isNaN(ageNum) || ageNum < 1 || ageNum > 18) {
    return res
      .status(400)
      .json({ error: "Age must be a number between 1 and 18" });
  }
                ...
                (err, existingEmail) => {
                  ...
                  bcrypt.hash(password, 10, (err, hash) => {
                    ...
                    const profileImagePath = profileImage
                      ? `/uploads/children/${profileImage.filename}`
                      : null;

                    const sql = `INSERT INTO children (name, username, email, password, parent_id, age, profile_image) VALUES (?, ?, ?, ?, ?, ?, ?)`;
                    db.query(
                      sql,
                      [
                        name,
